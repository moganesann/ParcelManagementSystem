...
<div class="fixed-bg">
<p style = "color : white" > <strong> Search User Profile </strong></p>
</div>


<div class="containers">
	<form action="viewUser.php" method="POST">
        <p> <strong> Enter User ID to Search </strong></p>
        <br>
        <input type="text" name="userID" placeholder="User ID"/>
        <br>
        <input class="input" type="submit" value ="View">
		<br><br>
...
$sql = "SELECT userID, usertype FROM users ORDER BY usertype";
$result = mysqli_query($link,$sql);
if (mysqli_num_rows($result) > 0) {
  echo "<table><tr><th> User ID </th><th> User Type </th></tr>";
  // output data of each row
  while($row = $result->fetch_assoc()) {
    echo "<tr><td>".$row["userID"]."</td><td>".$row["usertype"]."</td></tr>";
  }
  echo "</table>";
} else {
  echo "0 results";
