...
$sqlduration = "SELECT studentID,DATEDIFF(dateGoodsCollectRecipient,dateGoodsCollectOfficer) as totald FROM warden WHERE studentID='" . $_POST["id"] . "'";
$resultduration=mysqli_query($link,$sqlduration);
$valuesduration=mysqli_fetch_assoc($resultduration);
$numduration_rows=$valuesduration['totald'];
...
<div class="fixed-bg">
<p style = "color : white" > <strong>CALCULATION</strong></p>
</div>
...
<div class ="containers2">
   ...
   if(mysqli_num_rows($result) > 0){ while($row = mysqli_fetch_array($result)){ ?>
	...
	<tr>
		<td>Date of Goods Received (Student) :</td>
		<td>
		<?php
	...
	<br><br>
	<button class="input" onclick="window.location.href='goodslistCalculation.php'">Back</button>
	<?php
   }}}
?>
  </div>
		...
		<script>
            ...
            function generateQRCode() {
				var qrd = 'days;'
                var qrtext = '<?php echo $numduration_rows; ?> days';
