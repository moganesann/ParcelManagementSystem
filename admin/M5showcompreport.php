...
$sql="SELECT  COUNT(type) as totalDMGgoods FROM complaint WHERE type ='Damaged Goods'";
$result1 = mysqli_query($link, $sql); 

$sql="SELECT  COUNT(type) as totallostgoods FROM complaint WHERE type ='Lost Goods'";
$result2 = mysqli_query($link, $sql); 

$sql="SELECT  COUNT(type) as totalunder FROM complaint WHERE type ='Undelivered'";
$result3 = mysqli_query($link, $sql); 

$sql="SELECT  COUNT(type) as totalOthers FROM complaint WHERE type ='Others'";
$result4 = mysqli_query($link, $sql); 



$row = mysqli_fetch_assoc($result1); 
$totaldmg = $row['totalDMGgoods'];

$row = mysqli_fetch_assoc($result2); 
$totallost = $row['totallostgoods'];

$row = mysqli_fetch_assoc($result3); 
$totalunders = $row['totalunder'];

$row = mysqli_fetch_assoc($result4); 
$totalother = $row['totalOthers'];

$sqltype4 = "SELECT users.userID, users.username, complaint.type, complaint.comment, complaint.status FROM users INNER JOIN complaint ON users.userID = complaint.userID";
$result = mysqli_query($link, $sqltype4);
...
<div class="fixed-bg">
<div class = "container"> 
<p style = "color : white" > <strong>Total of Complaint</strong></p>
</div>
</div>
...
<div class="row">
  <div class="column left" >
    <h3> CONTENT </h3>
    <table border="1" width="80%" class="center"  bordercolor="black ";>
<tr>
		<td>damgages goods :</td>
		<td>
		<?php
		echo $totaldmg;
		?>
...
<br>
<button class="input input1" onclick="document.location='M5complaintreport.php'">View Report</button>
  </div>
